...
export default async function SearchPage({ searchParams }: Props) {
  ...
  const all = await getProducts(200);
  ...
  return (
    <section className="py-16 md:py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {!query ? (
          <p className="text-charcoal/60">Enter a product name or keyword above to search the catalogue.</p>
        ) : products.length ? (
          <ProductGrid products={products} />
        ) : (
          <div className="text-center py-16">
            <p className="text-charcoal/60 mb-6">No products found for “{q}”.</p>
